...
class AgentLoop:
    ...
    def __init__(
        ...
        # Determine scoped directories for specialist agents
        session_dir: Path | None = None
        memory_dir: Path | None = None
        if agent_name:
            from nanobot.utils.helpers import ensure_dir

            agent_base = ensure_dir(workspace / "agents" / agent_name)
            session_dir = ensure_dir(agent_base / "sessions")
            memory_dir = ensure_dir(agent_base / "memory")

        self.context = ContextBuilder(workspace, agent_profile=agent_profile, memory_dir=memory_dir)
        self.sessions = session_manager or SessionManager(workspace, session_dir=session_dir)
        ...
        while iteration < self.max_iterations:
            ...
            if response.usage:
                for k in ("prompt_tokens", "completion_tokens", "total_tokens"):
                    self.token_usage[k] += response.usage.get(k, 0)
