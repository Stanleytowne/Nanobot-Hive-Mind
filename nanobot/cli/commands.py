...
def gateway(
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        # Complex tasks: route through agent (or orchestrator in multi-agent mode)
        reminder_note = (
    ...
    if orch_cfg and orch_cfg.enabled:
        ...
        # Override heartbeat to route through orchestrator
        async def on_heartbeat_execute_orch(tasks: str) -> str:
            """Route heartbeat tasks through the orchestrator (multi-agent aware)."""
            from nanobot.bus.events import InboundMessage as HBInbound
            ...
            # Response is delivered via outbound bus by the orchestrator
            return ""

        heartbeat.on_execute = on_heartbeat_execute_orch
        # Suppress notify since orchestrator handles outbound directly
        heartbeat.on_notify = None

        # Override cron to route through orchestrator in multi-agent mode
        _original_on_cron_job = cron.on_job

        async def on_cron_job_orch(job: CronJob) -> str | None:
            """Route cron jobs through the orchestrator (multi-agent aware)."""
            from nanobot.bus.events import InboundMessage as CronInbound
